...
namespace OnlineStore.Areas.Identity.Pages.Account
{
    public class RegisterModel : PageModel
    {
        ...
        private OnlineStoreUser CreateUser()
        {
            try
            {
                return Activator.CreateInstance<OnlineStoreUser>();
            }
            catch
            {
                throw new InvalidOperationException($"Can't create an instance of '{nameof(OnlineStoreUser)}'. " +
                    $"Ensure that '{nameof(OnlineStoreUser)}' is not an abstract class and has a parameterless constructor, or alternatively " +
                    $"override the register page in /Areas/Identity/Pages/Account/Register.cshtml");
            }
        }

        private IUserEmailStore<OnlineStoreUser> GetEmailStore()
        {
            if (!_userManager.SupportsUserEmail)
            {
                throw new NotSupportedException("The default UI requires a user store with email support.");
            }
            return (IUserEmailStore<OnlineStoreUser>)_userStore;
        }
    }
}
